...
class EmbeddingEngine:
    ...
    def _create_onnx_session(self) -> None:
        """Instantiate the ONNXEmbedder."""
    ...
    def _log_import_warning(self) -> None:
        """Log a warning when qwen3-embed is not installed."""
        logger.warning(
    ...
    async def encode_batch(self, texts: list[str]) -> list[np.ndarray]:
        ...
        if not texts:
            return []
        return list(await asyncio.gather(*(self.encode(text) for text in texts)))
